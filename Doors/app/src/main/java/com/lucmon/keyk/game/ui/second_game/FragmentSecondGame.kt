package com.lucmon.keyk.game.ui.second_game
...
import com.lucmon.keyk.databinding.FragmentSecondGameBinding
...
class FragmentSecondGame : ViewBindingFragment<FragmentSecondGameBinding>(FragmentSecondGameBinding::inflate) {
    private val viewModel: SecondGameViewModel by viewModels()
    private val args: FragmentSecondGameArgs by navArgs()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.gesture.observe(viewLifecycleOwner) {
            binding.apply {
                when (it.first) {
                    1 -> {
                        selectRock.setBackgroundResource(R.drawable.bg_main_gradient)
                    ...
                    2 -> {
                        ...
                        selectPaper.setBackgroundResource(R.drawable.bg_main_gradient)
                        selectScissors.setBackgroundColor(resources.getColor(android.R.color.transparent))
                    }
                    else -> {
                        selectRock.setBackgroundColor(resources.getColor(android.R.color.transparent))
                        selectPaper.setBackgroundColor(resources.getColor(android.R.color.transparent))
                        selectScissors.setBackgroundResource(R.drawable.bg_main_gradient)
                    }
                }
            }
        }
        viewModel.scores.observe(viewLifecycleOwner) {
            ...
            if (it.first == 3) {
                ...
                findNavController().popBackStack(R.id.fragmentBid, inclusive = false)
            }
            if (it.second == 3) {
